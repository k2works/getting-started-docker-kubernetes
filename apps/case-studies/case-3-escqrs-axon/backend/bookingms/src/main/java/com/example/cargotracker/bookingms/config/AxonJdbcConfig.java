 ...
 * Jackson 3（{@code tools.jackson.databind.ObjectMapper}）を前提とした {@code defaultAxonObjectMapper}
 * を要求するため、Spring Boot 4 + Jackson 2 の組み合わせでは autoconfig が機能しない。本クラスで
 ...
 *       {@code JdbcTransactionalExecutorProvider.SUPPLIER_KEY} を bind する。
 *       これが無いと {@code JdbcTokenStore.connectionExecutor()} が
 *       「A connection executor must be present in the processing context」で失敗する。</li>
 *   <li>{@link TokenSchema} — snake_case 列名に統一（Flyway V003 と一致）</li>
