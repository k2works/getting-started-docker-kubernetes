...
/**
 * IT5 P1（HandlingActivity の {@code @CommandHandler} 欠落による Axon ルーティング失敗）
 * の再発を防ぐ ArchUnit ルール。
 *
 * <p>{@code domain.model.aggregates} 配下の Aggregate クラスに定義されたメソッドのうち、
 * いずれかの引数型が {@code *Command} で終わるものは {@link CommandHandler} アノテーションを
 * 必須とする。</p>
 *
 * <p>本ルールは IT6 で 4 サービス（bookingms / handlingms / routingms / trackingms）に
 * 共通化する。各サービスは同じ命名のテストクラスをパッケージのみ書き換えて配置する。</p>
 *
 * <p>参考: docs/development/retrospective-5.md P1 / docs/development/iteration_plan-6.md TI05-1.3</p>
 */
@AnalyzeClasses(packages = "com.example.cargotracker.bookingms.domain.model.aggregates")
    ...
    static final ArchRule commandHandlersMustBeAnnotated =
            classes()
                    ...
                    .because("Axon の Command ルーティングには @CommandHandler 注釈が必須 "
                            + "（IT5 P1 再発防止: HandlingActivity register/updateStatus 欠落事例）")
