...
/**
 * HandlingActivityController のメソッド認可テスト（IT10 A1.1 / US30）。
 *
 * <p>{@code @PreAuthorize("hasAnyRole('HANDLER', 'ADMIN')")} が
 * HerokuSecurityConfig の URL ルール認可と二段重層で機能することを slice テストで検証する。</p>
 */
@ExtendWith(SpringExtension.class)
@WebMvcTest(controllers = HandlingActivityController.class,
        excludeAutoConfiguration = {
                org.springframework.boot.autoconfigure.security.servlet.SecurityAutoConfiguration.class
        })
@Import(HandlingActivityControllerAuthorizationTest.TestMethodSecurityConfig.class)
class HandlingActivityControllerAuthorizationTest {
    ...
    void shouldReturn401WhenUnauthenticated() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/handling/act-1"))
    ...
    @Test
    @DisplayName("ROLE_HANDLER の GET は @PreAuthorize を通過し業務応答（404）を返す")
    @WithMockUser(roles = "HANDLER")
    void shouldPassPreAuthorizeWhenHandlerRole() throws Exception {
