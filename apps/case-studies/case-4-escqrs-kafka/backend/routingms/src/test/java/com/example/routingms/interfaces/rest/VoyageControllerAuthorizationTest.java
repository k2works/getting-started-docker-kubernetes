...
/**
 * VoyageController のメソッド認可テスト（IT10 A1.1 / US30）。
 *
 * <p>{@code @PreAuthorize("hasAnyRole('ROUTING', 'ADMIN')")} が
...
@Import(VoyageControllerAuthorizationTest.TestMethodSecurityConfig.class)
class VoyageControllerAuthorizationTest {
    ...
    void shouldReturn401WhenUnauthenticated() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/voyages/V001"))
    ...
    @Test
    @DisplayName("ROLE_ROUTING の GET は @PreAuthorize を通過し業務応答（404）を返す")
    @WithMockUser(roles = "ROUTING")
    void shouldPassPreAuthorizeWhenRoutingRole() throws Exception {
        // queryService.findByVoyageNumber がデフォルト null を返すため 404 になるが、
