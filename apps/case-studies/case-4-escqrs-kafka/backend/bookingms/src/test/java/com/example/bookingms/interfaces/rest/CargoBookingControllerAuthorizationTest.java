...
/**
 * CargoBookingController のメソッド認可テスト（IT10 A1.1 / US30）。
 *
 * <p>{@code @PreAuthorize("hasAnyRole('SALES', 'ADMIN')")} が
 * HerokuSecurityConfig の URL ルール認可と二段重層で機能することを slice テストで検証する。</p>
 */
@ExtendWith(SpringExtension.class)
@WebMvcTest(controllers = CargoBookingController.class,
        excludeAutoConfiguration = {
                org.springframework.boot.autoconfigure.security.servlet.SecurityAutoConfiguration.class
        })
@Import(CargoBookingControllerAuthorizationTest.TestMethodSecurityConfig.class)
class CargoBookingControllerAuthorizationTest {
    ...
    @Test
    @DisplayName("ROLE_SALES の GET は @PreAuthorize を通過し業務応答（404）を返す")
    @WithMockUser(roles = "SALES")
    void shouldPassPreAuthorizeWhenSalesRole() throws Exception {
        // queryService.findCargoById がデフォルト null を返すため 404 になるが、
