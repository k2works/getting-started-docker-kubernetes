 ...
 *
 * <p>5 ms 目の実装（A1.4 完遂）。trackingms の default SecurityFilterChain（{@code /api/v1/tracking/**}）
 * 専用。public chain（{@code /api/v1/public/tracking/**}）は {@link PublicTrackingTokenFilter}
 * による時限署名 JWT 検証で代替認証するため本フィルタは適用しない。</p>
 *
 * <p>IT11 で {@code shared-security} モジュール抽出（ADR-0022 起票候補、IT9 M1）。</p>
 */
