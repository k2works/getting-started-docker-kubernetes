...
class PreAuthFilterTest {
    ...
    void shouldBuildAuthenticationWhenBothHeadersPresent() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/v1/tracking/TN-001");
        request.addHeader(PreAuthFilter.HEADER_USER, "alice");
        request.addHeader(PreAuthFilter.HEADER_ROLE, "TRACKER");
    ...
    void shouldSplitMultipleRoles() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/v1/tracking/TN-001");
        request.addHeader(PreAuthFilter.HEADER_USER, "alice");
        request.addHeader(PreAuthFilter.HEADER_ROLE, "TRACKER, ADMIN");
        MockHttpServletResponse response = new MockHttpServletResponse();
        FilterChain chain = new MockFilterChain();
        ...
        assertThat(authentication.getAuthorities())
                .extracting(GrantedAuthority::getAuthority)
                .containsExactly("ROLE_TRACKER", "ROLE_ADMIN");
    }
    ...
    void shouldKeepExistingRolePrefix() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/v1/tracking/TN-001");
        request.addHeader(PreAuthFilter.HEADER_USER, "alice");
        request.addHeader(PreAuthFilter.HEADER_ROLE, "ROLE_TRACKER");
        MockHttpServletResponse response = new MockHttpServletResponse();
        FilterChain chain = new MockFilterChain();
